...
export default function ClientPage() {
  ...
  return (
    <div className="min-h-screen">
      <nav className="flex justify-between items-center px-8 py-4 border-b border-white/5 sticky top-0 z-50 bg-ink/90 backdrop-blur-xl">
        <div
        ...
        {searched && (
          <div>
            ...
            <div className="space-y-4">
              {artists.map((artist) => (
                <div key={artist.id} className="card p-5 flex items-start gap-4">
                  <div className="w-12 h-12 rounded-full bg-surface2 border border-white/10 flex items-center justify-center text-white/30 text-lg shrink-0">
                    {artist.name?.[0]?.toUpperCase() ?? '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium mb-0.5">{artist.name}</p>
                    ...
                    {artist.styles?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {artist.styles.slice(0, 4).map((s: string) => (
                          <span
                            key={s}
                            className="px-2.5 py-1 rounded-full bg-surface2 text-white/50 text-xs"
                          >
                            {s}
                          </span>
                        ))}
